...
export class SupabaseSignalStore implements SignalStore {
  ...
  async trackRecord(): Promise<TrackRecordReport> {
    ...
    if (kindByKey.size > 0) {
      ...
      for (let i = 0; i < keys.length; i += 500) {
        ...
        for (const o of outs ?? []) {
          ...
          const t = tallies.get(key) ?? emptyTally(kind, o.horizon);
          // The stamps arrive as `timestamptz`; `ms` is the same conversion
          // the read path uses, and a column the database left NULL stays
    ...
    // Rows and prose are shared with the memory store — see the header of
    // `trackRecordRows.ts` for why keeping two copies stopped being viable.
