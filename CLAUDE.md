...
- Tests cover the flow engine (`quantflow-modules/flow-engine`, 14 tests) and the backend seams (`backend/test`, 36 tests: the adapter, the health-error formatter, the enrichment latch, and the demo-mode gate) — both `node:test` via `tsx`. `frontend/` has none. Verify frontend changes and route behaviour by running the dev servers and exercising the affected pages manually.
