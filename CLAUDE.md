...
- **`backend/src/domain/`** and **`backend/src/flow-engine/`** are VENDORED copies, compiled into the backend rather than imported as packages. Render builds the backend with `rootDir: backend` and starts `node dist/server.js`, so a `file:../packages/domain` dependency would resolve `main: dist/index.js` to nothing — nothing in that build compiles a sibling package. `scripts/sync-vendored.mjs` regenerates `src/domain/` from `packages/domain/src/` (stripping `.js` import extensions for CJS); run `npm run sync:vendor` after editing the canonical source. **`backend/test/vendorDrift.test.ts` fails if either copy drifts**, so the long-standing "mirror it to the module" instruction is now enforced rather than merely stated. The flow-engine copy is permitted exactly one documented difference — the added `resetDaily()` — asserted as a literal, and separately asserted to actually be present so the allowance is not vacuous.
