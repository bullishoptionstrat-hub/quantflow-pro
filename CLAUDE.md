...
- **HTTP CORS is now an allowlist** (`[FRONTEND_URL, 'http://localhost:3000']`, `server.ts:43`); browser traffic reaches the backend through the frontend's `/api/*` rewrite proxy, which is server-to-server and sends no `Origin`. **Socket.IO still allows `origin: '*'` with `credentials: true` and has no authentication handshake** (`server.ts:31-36`) — so the six REST routes behind `requireAuth` are bypassable by subscribing to the socket, which broadcasts the same `flow_batch`/macro/sentiment/news/cboe payloads. See `docs/FORENSIC_AUDIT.md` #29 before relying on route auth for anything security-sensitive.
