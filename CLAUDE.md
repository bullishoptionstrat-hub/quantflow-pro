...
- Tests cover the flow engine (`quantflow-modules/flow-engine`, 14 tests), `@quantflow/domain` (`packages/domain`, 45 tests), the backend (`backend/test`, 327 tests) and the frontend (`frontend`, 29 tests under vitest) — the first three under `node:test` via `tsx`. `npm run verify` at the repo root runs every suite plus both builds and the vendor drift check. Verify frontend changes and route behaviour by running the dev servers and exercising the affected pages manually.
