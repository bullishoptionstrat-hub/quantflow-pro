...
- **`backend/src/domain/`**, **`backend/src/flow-engine/`**, and **`backend/src/enrichment/firecrawl/`** are VENDORED copies, compiled into the backend rather than imported as packages. Render builds the backend with `rootDir: backend` and starts `node dist/server.js`, so a `file:../packages/domain` dependency would resolve `main: dist/index.js` to nothing — nothing in that build compiles a sibling package. The canonical sources are ESM and the backend is CJS, so each copy is generated with relative-import extensions stripped. `scripts/sync-vendored.mjs` regenerates `src/domain/` from `packages/domain/src/`; run `npm run sync:vendor` after editing the canonical source. **`backend/test/vendorDrift.test.ts` fails if a copy drifts**, so the long-standing "mirror it to the module" instruction is enforced rather than merely stated. The flow-engine copy is permitted exactly one documented difference — the added `resetDaily()` — asserted as a literal, and separately asserted to actually be present so the allowance is not vacuous.
...
- Tests cover the flow engine (`quantflow-modules/flow-engine`, 14 tests), `@quantflow/domain` (`packages/domain`, 45 tests), the backend (`backend/test`, 344 tests) and the frontend (`frontend`, 29 tests under vitest) — the first three under `node:test` via `tsx`. `npm run verify` at the repo root runs every suite plus both builds and the vendor drift check. Verify frontend changes and route behaviour by running the dev servers and exercising the affected pages manually.
...
- **Both HTTP and Socket.IO use the same CORS allowlist** (`[FRONTEND_URL, 'http://localhost:3000']`) and **the socket handshake is authenticated** (`middleware/socketAuth.ts`, installed in `server.ts` before any connection handler). HTTP browser traffic arrives via the frontend's `/api/*` rewrite proxy (server-to-server, no `Origin`); socket traffic goes direct, so its allowlist is load-bearing. The client sends the Supabase access token via socket.io's `auth` callback, which re-reads it on every reconnect so an expired token doesn't look like a random logout. Handshake auth **fails closed** — including when the identity provider is unreachable, since an unverifiable connection must not receive the stream. Demo mode is the one unauthenticated admission, under the same two independent conditions the HTTP path uses (`DEMO_MODE=1` **and** `demo: '1'` in the handshake `auth`); only a *tokenless* handshake is demo-admissible, so a token that fails to verify is never downgraded into a demo session. Every broadcast event is a feed `requireAuthOrDemo` already admits — the chain and enrichment endpoints are not broadcast at all. Was finding #29: route auth had been bypassable by subscribing to the socket.
