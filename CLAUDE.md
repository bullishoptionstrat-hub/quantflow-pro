...
- Tests cover the flow engine (`quantflow-modules/flow-engine`, 14 tests) and the backend seams (`backend/test`, 112 tests: the adapter, the health-error formatter, the enrichment latch, the demo-mode gate, plus the rights registry, signal identity/decision time, the store, the recorder, the grader, and an end-to-end history integration suite) — both `node:test` via `tsx`. `npm run verify` in `backend/` runs typecheck (src **and** test, via `tsconfig.test.json`) then the suite. `frontend/` has none. Verify frontend changes and route behaviour by running the dev servers and exercising the affected pages manually.
