...
| R-3 | **Rights taint does not propagate past the API boundary** (§22, F-16). **Measured:** `SignalRecord` carries `source`/`datasetId`/`rightsClass`; `FlowEvent` carries none of them, so `/api/flow`, the socket and the CSV export drop the lineage the store keeps. | `SILENT` | none | egress gates at serve/export/train (§23) | absent | **OPEN — inert today (the connector gate refuses PROHIBITED before `start()`), live the moment a licensed feed connects** |
