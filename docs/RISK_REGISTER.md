...
| R-3 | **Rights taint past the API boundary** (§22, F-16). Wire and CSV now carry `datasets` + `rights_display` (weakest DISPLAY class across contributing sources). | `SILENT` | lineage is on every row | **carried**, not yet **gated** — §23 wants six gates and there are two | `trackRecordWire.test.ts` (10) | **PARTLY CLOSED** — lineage travels; serve/export/train still do not refuse on it |
